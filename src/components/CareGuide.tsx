...
export const CareGuide: React.FC = () => {
  ...
  const careDays: CareDayInfo[] = [
    ...
    },
    {
      ...
      warning: '⚠️ 가장 중요: 간지럽다고 손으로 딱지나 각질을 억지로 떼어내면 색소가 같이 떨어져 얼룩이 생깁니다. 자연 탈각되도록 절대 손대지 마세요.',
    },
    {
      day: 'Day 6~7',
  ...
  return (
    <div className="card">
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: '16px' }}>
        <Calendar size={20} color="var(--color-text-muted)" style={{ marginRight: '8px' }} />
        <span style={{ fontSize: '15px', fontWeight: 700 }}>시술 후 7일 케어 캘린더</span>
      </div>
        ...
        {careDays.map((c, i) => (
          <button
            ...
            style={{
              ...
              border: 'none',
              cursor: 'pointer',
              backgroundColor: selectedDay === i ? 'var(--color-card)' : 'transparent',
              color: selectedDay === i ? 'var(--color-text-main)' : 'var(--color-text-muted)',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '16px' }}>
          {careDays[selectedDay].checklist.map((item, idx) => (
            <div key={idx} style={{ display: 'flex', alignItems: 'flex-start', gap: '8px' }}>
              ...
              />
              <span style={{ fontSize: '13px', lineHeight: '1.5', color: 'var(--color-text-main)' }}>
                {item}
              </span>
            </div>
          ))}
